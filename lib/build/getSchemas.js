...
async function parseSchema(schemasPath, filename) {
  ...
  const newFilename = filename.replace(ext, `.html`);
...
async function getSchemas(schemasPath) {
  ...
  return schemas
  .filter(schemaInfo => schemaInfo)
  .filter(schemaInfo => schemaInfo.schema)
  .sort((a, b) => compare(a.schema.title, b.schema.title));
